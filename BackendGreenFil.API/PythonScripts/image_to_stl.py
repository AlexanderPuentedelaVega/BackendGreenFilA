...
def main(image_path, output_dir, output_name):
    ...
    while True:
        ...
        if estado == "SUCCEEDED":
            ...
            #CAMBIO - 10/06
            rotated_filename = os.path.join(output_dir, f"{sys.argv[3]}.stl")
            your_mesh.save(rotated_filename)
        
            # GLB (requiere pip install trimesh)
            mesh_trimesh = trimesh.load(rotated_filename)
            image_name = os.path.splitext(os.path.basename(image_path))[0]
            output_dir = sys.argv[2]
            os.makedirs(output_dir, exist_ok=True)
            glb_filename = os.path.join(output_dir, f"{image_name}.glb")
            mesh_trimesh.export(glb_filename)
        
            # Limpiar temporales si deseas
